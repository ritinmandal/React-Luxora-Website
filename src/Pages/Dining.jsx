...
export default function DiningPagePremium() {
  ...
  const cuisines = useMemo(() => {
    const set = new Set(dishes.map((d) => normalize(d.cuisine)).filter(Boolean));
    return ['All', ...Array.from(set).sort()];
  }, [dishes]);

  const filtered = useMemo(() => {
    if (normalize(activeCuisine) === 'all') return dishes;
    return dishes.filter((d) => normalize(d.cuisine) === normalize(activeCuisine));
  ...
  return (
    <Box sx={{ backgroundColor: (t) => (t.palette.mode === 'dark' ? '#0f0f0f' : '#f7f7f7'), color: 'text.primary' }}>
      <Box
        sx={{
          position: 'relative',
          height: { xs: '60vh', md: '75vh' },
          backgroundImage:
            "url('https://images.pexels.com/photos/5591660/pexels-photo-5591660.jpeg?auto=compress&cs=tinysrgb&w=1920')",
      ...
      >
        <Box
          sx={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(180deg, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.35) 40%, rgba(0,0,0,0.75) 100%)',
          }}
        />
        <Container sx={{ position: 'relative', zIndex: 2 }}>
          <Stack spacing={2} alignItems="center" textAlign="center">
            ...
            >
              Powered directly by your Supabase data – Indian, Chinese, Italian & more.
            </MotionTypography>
          </Stack>
        </Container>
      </Box>
        ...
        <MotionBox variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }}>
          ...
          >
            {loading ? 'Loading cuisines…' : `${filtered.length} dish${filtered.length !== 1 ? 'es' : ''} ${activeCuisine === 'All' ? 'available' : `in ${activeCuisine}`}`}
          </MotionTypography>

          <Stack direction="row" alignItems="center" justifyContent="center" spacing={1} flexWrap="wrap" sx={{ mb: { xs: 4, md: 6 } }}>
            <Chip icon={<FilterListIcon />} label="Filter" sx={{ borderColor: gold, color: gold, borderRadius: 999, mr: 1 }} variant="outlined" />
            {cuisines.map((c) => (
              <Chip
                key={c}
                label={c.charAt(0).toUpperCase() + c.slice(1)}
                onClick={() => setActiveCuisine(c)}
                color={activeCuisine.toLowerCase() === c ? 'warning' : 'default'}
                sx={{
                  borderRadius: 999,
                  px: 1.5,
                  borderColor: gold,
                  color: activeCuisine.toLowerCase() === c ? '#000' : gold,
                  backgroundColor: activeCuisine.toLowerCase() === c ? gold : 'transparent',
                  '&:hover': {
                    borderColor: gold,
                    backgroundColor: activeCuisine.toLowerCase() === c ? gold : 'rgba(191,162,108,0.08)',
                  },
                }}
                variant="outlined"
              />
